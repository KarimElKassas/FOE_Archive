...
class _MContextMenuState extends State<MContextMenu> {
  ...
  Widget build(BuildContext context) {
    ...
    double leftOnArabic = widget.position.dx ;
    double leftOnEnglish = widget.position.dx - widget.width;

    double rightOnArabic = MediaQuery.sizeOf(context).width - widget.position.dx - widget.width;
    double rightOnEnglish = MediaQuery.sizeOf(context).width - widget.position.dx ;

    double paddingLeft = Constants.currentLocale == LanguageType.ARABIC.getValue() ? leftOnArabic : leftOnEnglish;
    double paddingTop = widget.position.dy + 24;
    ...
    double paddingBottom = MediaQuery.sizeOf(context).height - widget.position.dy - height;
    if (paddingBottom < 0) {
